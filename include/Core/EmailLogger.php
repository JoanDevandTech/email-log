...
class EmailLogger implements Loadie {
	...
	public function log_email( $original_mail_info ) {
		...
		// Attachments detection and names.
		$attachments_list = array();
		if ( ! empty( $mail_info['attachments'] ) ) {
			$attachments_list = is_array( $mail_info['attachments'] ) ? $mail_info['attachments'] : array( $mail_info['attachments'] );
			$attachments_list = array_filter( array_map( 'strval', $attachments_list ) );
		}
		if ( ! empty( $attachments_list ) ) {
			$log['attachments']     = 'true';
			$log['attachment_name'] = implode( ', ', array_map( 'basename', $attachments_list ) );
		} else {
